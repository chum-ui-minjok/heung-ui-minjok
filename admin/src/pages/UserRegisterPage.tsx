...
const UserRegisterPage = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(
        `어르신이 성공적으로 등록되었습니다! (ID: ${response.id})`
      );
      ...
      const errorMessage =
        err instanceof Error ? err.message : "어르신 등록에 실패했습니다.";
  ...
  const handleCancel = () => {
    if (
      window.confirm("정말 취소하시겠습니까? 입력한 내용이 모두 사라집니다.")
    ) {
      navigate(-1);
    }
  };
      ...
      <AdminLayout navItems={navigationItems}>
        <div style={{ maxWidth: "600px", margin: "0 auto", padding: "32px" }}>
          <div style={{ textAlign: "center", marginBottom: "32px" }}>
            ...
            <Button
              variant="primary"
              onClick={() => navigate("/dashboard/admin/device-register")}
            >
              기기 등록하기
            </Button>
          </div>
  ...
  const showNoDeviceWarning =
    !isLoadingDevices && devices.length === 0 && !deviceLoadError;
